# r3v27hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if atm in state.order_depths:
            ...
            if atm_d.buy_orders and atm_d.sell_orders:
                ...
                for w in wings:
                    ...
                    if d.buy_orders and d.sell_orders:
                        ...
                        if mid_w > 1:
                            if w not in result: result[w] = []
                            if atm not in result: result[atm] = []

                            result[w].append(Order(w, wbid, -4))
                            result[atm].append(Order(atm, aask, 4))
