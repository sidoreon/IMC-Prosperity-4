...
pub fn workspace_root() -> PathBuf {
    if let Ok(raw) = std::env::var("PROSPERITY_WORKSPACE_ROOT") {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
            let p = PathBuf::from(trimmed);
            if p.is_dir() && p.join("datasets").is_dir() {
                return p;
            }
        }
    }
    ...
    let mut dir = crate_root.as_path();

    // Monorepo layout: the crate may live in `backtester/` while `datasets/` and `traders/` sit
    // in the parent directory. Walk up a few ancestors (covers deeper nesting too).
    for _ in 0..8 {
        if dir.join("datasets").is_dir()
            || dir.join("traders").is_dir()
            || dir.join("opensource").is_dir()
        {
...
mod tests {
    use super::{
        ...
        match_orders_for_symbol_raw_csv_tape, position_limit, python_round_to_digits,
    ...
    #[test]
    fn round3_products_use_wiki_position_limits() {
    ...
    fn log_only_mode_writes_submission_log_and_metrics() {
        ...
        let request = RunRequest {
            trader_file: workspace_root().join("traders/latest_trader.py"),
            dataset_file: workspace_root().join("datasets/tutorial/prices_round_0_day_-1.csv"),
...
"#,
    ...
    fn display_path_prefers_project_relative_paths() {
        let path = workspace_root().join("datasets/tutorial/prices_round_0_day_-1.csv");
        assert_eq!(
            display_path(&path),
