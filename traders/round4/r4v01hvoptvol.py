# r4v01hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r4v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        self._track_counterparties(data, state)
    ...
    def _track_counterparties(self, data: dict, state: TradingState) -> None:
        # Decay existing counterparty activity scores, then bump on fresh market_trades.
        CP_DECAY = 0.80
        cp = data.get("cp", {})
        for k in list(cp.keys()):
            cp[k] = float(cp[k]) * CP_DECAY
        ...
        for product, trades in market_trades.items():
            for tr in (trades or []):
                ...
                if product == "HYDROGEL_PACK":
                    if any("Mark 38" in p for p in participants):
                        cp["m38_hg"] = 1.0

                if product == "VELVETFRUIT_EXTRACT":
                    if any("Mark 22" in p for p in participants):
                        cp["m22_ve"] = 1.0
                    if any("Mark 55" in p for p in participants):
                        cp["m55_ve"] = 1.0
                    if any("Mark 67" in p for p in participants):
                        cp["m67_ve"] = 1.0
                ...
                if product in ("VEV_6000", "VEV_6500"):
                    if any("Mark 01" in p for p in participants):
                        cp["m01_6x"] = 1.0
    ...
    def _trade_hydrogel(self, depth: OrderDepth, position: int, data: dict) -> List[Order]:
        ...
        post_edge = self.HG_POST_EDGE
        if data.get("cp", {}).get("m38_hg", 0.0) > 0.25:
            post_edge = 15

        skew = position // self.HG_SKEW_DIV
        bid_px = int(fair - post_edge - skew)
        ask_px = int(fair + post_edge - skew)
    ...
    def _extract_fair(self, data: dict, depth: OrderDepth, mids: Dict[str, float], micros: Dict[str, float], pos: int) -> float:
        ...
        cp = data.get("cp", {})
        if cp.get("m67_ve", 0.0) > 0.25:
            fair += 1.0
        elif cp.get("m22_ve", 0.0) > 0.25:
            fair -= 1.0
    ...
    def _trade_delta_one(self, product: str, depth: OrderDepth, position: int, limit: int, data: dict, fair: float) -> List[Order]:
        ...
        post_edge = self.VE_POST_EDGE
        if data.get("cp", {}).get("m55_ve", 0.0) > 0.25:
            post_edge = self.VE_POST_EDGE + 1
    ...
    def _trade_voucher(self, product: str, depth: OrderDepth, pos: int, fair: float, underlying_fair: float, delta: float, data: dict) -> List[Order]:
        ...
        if data.get("cp", {}).get("m22_vox", 0.0) > 0.25:
            buy_take_edge += 1.5
            buy_quote_edge += 1.0
        ...
        if K >= 6000 and not lock_only_unwind:
            cp = data.get("cp", {})
            cap_long = int(0.40 * limit)
            buy_cap = limit - pos
            sell_cap = limit + pos

            if cp.get("m22_vox", 0.0) > 0.25 and buy_cap > 0 and pos < cap_long:

                qty = min(buy_cap, 20, cap_long - pos)
                if qty > 0:
                    orders.append(Order(product, 1, qty))
                    buy_cap -= qty

            if cp.get("m01_6x", 0.0) > 0.25 and pos > 0 and sell_cap > 0:

                flip_px = max(2, (best_bid or 1) + 1)
                qty = min(pos, sell_cap, 20)
                if qty > 0:
                    orders.append(Order(product, flip_px, -qty))
    ...
    def _load_data(self, trader_data: str) -> dict:
        if not trader_data:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "cp": {}}
        try:
