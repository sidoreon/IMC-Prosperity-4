# r2v03osmiuminv.py: vNN counts only inside the `osmiuminv` family (same round can have other r2v01… files with different tags).
...
class OsmiumStrategy:
    ...
    def run(self, order_depth: OrderDepth, position: int, history: List[float], timestamp: int):
        ...
        if position < -self.REDUCE_TRIGGER and z > 0:
        ...
        if aggressive_mode or abs(z) > self.LOW_SIGNAL_Z:
            ...
            if mid <= rolling_mean - edge and buy_cap > 0:
                qty = min(ask_qty, buy_cap, size)
                if qty > 0:
                    orders.append(Order(self.PRODUCT, best_ask, qty))

            if mid >= rolling_mean + edge and sell_cap > 0:
                qty = min(bid_qty, sell_cap, size)
                if qty > 0:
        ...
        print(f"[OSMIUMv3] t={timestamp} mid={mid:.2f} fair={fair:.2f} z={z:.2f} rmean={rolling_mean:.2f} pos={position} spread={spread}")
