# r3v15hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    VE_ANCHOR_WEIGHT = 0.62
    VE_EMA_ALPHA = 0.13
    VE_IMBALANCE_K = 1.5
    VE_IMBALANCE_CAP = 3.0
    ...
    VOUCHER_FAIR_BLEND = 0.25
    VOUCHER_FAIR_ALPHA = 0.15
    ...
    def run(self, state: TradingState):
        ...
        _v_ema: Optional[float] = None
        _v_blend: float = 0.0
        _v_raw, _v_conf = self._voucher_implied_fair(state.order_depths)
        if _v_raw is not None and _v_conf > 0:
            _prev_vf = float(data.get("_vf_ema", _v_raw))
            _v_ema = self.VOUCHER_FAIR_ALPHA * _v_raw + (1.0 - self.VOUCHER_FAIR_ALPHA) * _prev_vf
            data["_vf_ema"] = _v_ema
            _v_blend = _v_conf * self.VOUCHER_FAIR_BLEND

        if extract_fair is not None and _v_ema is not None:
            extract_fair = (1.0 - _v_blend) * extract_fair + _v_blend * _v_ema
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "VELVETFRUIT_EXTRACT":
                ...
                if _v_ema is not None:
                    fair = (1.0 - _v_blend) * fair + _v_blend * _v_ema
    ...
    def _voucher_implied_fair(self, order_depths: Dict[str, "OrderDepth"]) -> "Tuple[Optional[float], float]":
        # Breeden-Litzenberger estimate of VELVETFRUIT_EXTRACT fair value. For call options: dC/dK = -(P(S > K)).
        strike_mids = []
        for product in self.LIQUID_VOUCHERS:
            ...
            if mid is None or mid <= 0:
                continue
            strike_mids.append((K, mid))

        if len(strike_mids) < 3:
            return None, 0.0

        strike_mids.sort(key=lambda x: x[0])

        slopes = []
        for i in range(len(strike_mids) - 1):
            K1, C1 = strike_mids[i]
            K2, C2 = strike_mids[i + 1]
            slope = (C2 - C1) / (K2 - K1)
            mid_k = (K1 + K2) / 2.0
            slopes.append((mid_k, slope))

        S_est = None
        for i in range(len(slopes) - 1):
            mk_a, sl_a = slopes[i]
            mk_b, sl_b = slopes[i + 1]
            if sl_a <= -0.5 <= sl_b:
                denom = sl_b - sl_a
                if abs(denom) < 1e-9:
                    S_est = (mk_a + mk_b) / 2.0
                    ...
                    t = (-0.5 - sl_a) / denom
                    S_est = mk_a + t * (mk_b - mk_a)
                break

        if S_est is None:
            return None, 0.0

        confidence = min(1.0, len(strike_mids) / 5.0)
        return S_est, confidence
