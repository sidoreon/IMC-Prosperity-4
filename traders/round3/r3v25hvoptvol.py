# r3v25hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def _trade_hydrogel(self, depth: OrderDepth, position: int, data: dict) -> List[Order]:
        ...
        original_bid = int(fair - self.HG_POST_EDGE)
        original_ask = int(fair + self.HG_POST_EDGE)
        INV_K = 0.05
        inv_shift = int(INV_K * position)
        bid_px = original_bid - inv_shift
        ask_px = original_ask - inv_shift
    ...
    def _trade_delta_one(self, product: str, depth: OrderDepth, position: int, limit: int, data: dict, fair: float) -> List[Order]:
        ...
        original_bid = int(fair - self.VE_POST_EDGE)
        original_ask = int(fair + self.VE_POST_EDGE)
        INV_K = 0.05
        inv_shift = int(INV_K * position)
        bid_px = original_bid - inv_shift
        ask_px = original_ask - inv_shift
    ...
    def _trade_voucher(self, product: str, depth: OrderDepth, pos: int, fair: float, underlying_fair: float, delta: float, data: dict) -> List[Order]:
        ...
        if spread >= min_quote_spread and not (fair <= 1.05 and spread < 3):
            ...
            original_bid = math.floor(fair - buy_quote_edge)
            original_ask = math.ceil(fair + sell_quote_edge)
            INV_K = 0.05
            inv_shift = int(INV_K * pos)
            bid_px = original_bid - inv_shift
            ask_px = original_ask - inv_shift

            bid_px = min(best_bid + 1, bid_px)
            ask_px = max(best_ask - 1, ask_px)
