# r2v03peposmpair.py: vNN counts only inside the `peposmpair` family (same round can have other r2v01… files with different tags).
        ...
        for product in self.PRODUCTS:
            ...
            if product == "INTARIAN_PEPPER_ROOT":
                ...
                if make_bid < make_ask:
                    can_buy = min(max_buy - buy_allocated, self.bid())
                    if can_buy > 0:
                        orders.append(Order(product, make_bid, can_buy))
                    can_sell = min(max_sell - sell_allocated, self.bid())
                    if can_sell > 0:
