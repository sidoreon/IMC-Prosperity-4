# r2v04osmiuminv.py: vNN counts only inside the `osmiuminv` family (same round can have other r2v01… files with different tags).
...
class OsmiumStrategy:
    ...
    def run(self, order_depth: OrderDepth, position: int, history: List[float], timestamp: int):
        ...
        print(f"[OSMIUMv4] t={timestamp} mid={mid:.2f} fair={fair:.2f} z={z:.2f} ts={take_size} pos={position} spread={spread}")
