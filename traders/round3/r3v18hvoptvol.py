# r3v18hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            elif product in ("VEV_6000", "VEV_6500") and extract_fair is not None:
                fair = self._voucher_fair(product, extract_fair, state.timestamp, data)
                orders = self._trade_wide_wing_voucher(product, depth, pos, fair, extract_fair, state.timestamp, data)
    ...
    def _sigma_for_product(self, data: dict, product: str, underlying_fair: Optional[float] = None, timestamp: Optional[int] = None) -> float:
        # Annualized BS sigma from the observed cross-day volatility smile. The smile fit predicts total volatility v_t = sig...
    ...
    def _update_voucher_sigma(self, data: dict, order_depths: Dict[str, OrderDepth], underlying_fair: float, timestamp: int) -> None:
        # Update only a small residual around the fixed cross-day smile. This avoids relearning the full smile from a single ...
    ...
    def _trade_wide_wing_voucher(self, product: str, depth: OrderDepth, pos: int, fair: float,
                                 underlying_fair: float, timestamp: int, data: dict) -> List[Order]:
        # Wide market making only for VEV_6000 and VEV_6500.
        orders: List[Order] = []
        limit = self.POSITION_LIMITS[product]
        best_bid, bid_vol = self._best_bid(depth)
        best_ask, ask_vol = self._best_ask(depth)
        ...
        total_pnl = float(data.get("last_total_pnl", 0.0))
        peak_pnl = float(data.get("peak_pnl", total_pnl))
        lock_only_unwind = peak_pnl >= 8000.0

        buy_cap = limit - pos
        sell_cap = limit + pos

        if not lock_only_unwind and buy_cap > 0 and best_ask <= self._wing_max_buy_price(fair):
            qty = min(buy_cap, abs(ask_vol), self._wing_take_size(product, pos, side=1))
            if qty > 0:
                orders.append(Order(product, int(best_ask), int(qty)))
                buy_cap -= qty
                pos += qty

        if sell_cap > 0 and best_bid >= max(1, math.ceil(fair + 0.80)):
            qty = min(sell_cap, abs(bid_vol), self._wing_take_size(product, pos, side=-1))
            if qty > 0:
                orders.append(Order(product, int(best_bid), -int(qty)))
                sell_cap -= qty
                pos -= qty
        ...
        if lock_only_unwind:
            if pos > 0 and sell_cap > 0 and best_bid >= 0:
                qty = min(pos, sell_cap, abs(bid_vol), self._wing_take_size(product, pos, side=-1))
                if qty > 0:
                    orders.append(Order(product, int(best_bid), -int(qty)))
            return orders
        ...
        bid_px = 0
        ask_px = 1
        buy_size = min(buy_cap, self._wing_quote_size(product, pos, side=1))
        sell_size = min(sell_cap, self._wing_quote_size(product, pos, side=-1))

        if fair > self._wing_safe_short_fair(product, underlying_fair, timestamp):
            sell_size = min(sell_size, max(0, pos))

        if pos > 0.70 * limit:
            buy_size = 0
            sell_size = min(sell_cap, max(sell_size, 35))
        elif pos < -0.70 * limit:
            sell_size = 0
            buy_size = min(buy_cap, max(buy_size, 35))

        if buy_size > 0 and bid_px < best_ask:
            orders.append(Order(product, bid_px, int(buy_size)))
        if sell_size > 0 and ask_px > best_bid:
            orders.append(Order(product, ask_px, -int(sell_size)))

        return orders

    def _wing_max_buy_price(self, fair: float) -> int:
        return 1 if fair >= 1.85 else 0

    def _wing_safe_short_fair(self, product: str, underlying_fair: float, timestamp: int) -> float:
        day_progress = self._clip(timestamp / 1_000_000.0, 0.0, 1.0)
        base = 0.45 - 0.15 * day_progress
        if product == "VEV_6000":
            base -= 0.10
        if underlying_fair > self.VOUCHER_STRIKES[product] - 220:
            base *= 0.50
        return max(0.15, base)

    def _wing_quote_size(self, product: str, pos: int, side: int) -> int:
        limit = self.POSITION_LIMITS[product]
        base = 28 if product == "VEV_6000" else 34
        inv = pos / max(1, limit)
        if side == 1:
            factor = 1.0 - 0.85 * max(0.0, inv) + 0.45 * max(0.0, -inv)
        else:
            factor = 1.0 - 0.85 * max(0.0, -inv) + 0.45 * max(0.0, inv)
        return max(1, int(round(base * self._clip(factor, 0.15, 1.45))))

    def _wing_take_size(self, product: str, pos: int, side: int) -> int:
        base = 18 if product == "VEV_6000" else 22
        return min(base, self._wing_quote_size(product, pos, side))
