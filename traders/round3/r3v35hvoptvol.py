# r3v35hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        result.update(self._voucher_mm(state, vev_mid_hist))

        trader_data = json.dumps({
            "hydrogel_ema": hydrogel_ema,
            "last_hedge_timestamp": last_hedge_ts,
            "vev_mid_hist": vev_mid_hist,
        })

        return result, 0, trader_data

    def _hydrogel(
        self, od: OrderDepth, pos: int, ema: float, ts: int
        ...
        ANCHOR = 10000.0
        anchor_gap = mid - ANCHOR

        top_bid_vol = od.buy_orders[best_bid]
        top_ask_vol = -od.sell_orders[best_ask]
        total_vol = top_bid_vol + top_ask_vol
        imbalance = (top_bid_vol - top_ask_vol) / total_vol if total_vol > 0 else 0.0

        day_frac = (ts % 1_000_000) / 1_000_000
        ...
        if day_frac >= 0.75:
            ...
            if anchor_gap > 25:
                ...
                target = int(-anchor_gap * 4)
                target = max(-150, min(150, target))
        elif day_frac < 0.10:

            target = int(imbalance * 30)
        else:

            anchor_signal = -anchor_gap * 3.0
            imb_signal = imbalance * 20
            target = int(anchor_signal + imb_signal)
            target = max(-160, min(160, target))

        buy_cap = max(0, HYDROGEL_LIMIT - pos)
        sell_cap = max(0, HYDROGEL_LIMIT + pos)
        gap = target - pos

        orders: List[Order] = []

        taker_size = min(40, abs(gap))
        ...
        ask_px = best_ask - 1
        if bid_px < ask_px:
            if buy_cap > 0 and gap > -30:
                orders.append(Order(HYDROGEL, bid_px, min(passive_size, buy_cap)))
            if sell_cap > 0 and gap < 30:
                orders.append(Order(HYDROGEL, ask_px, -min(passive_size, sell_cap)))

        return orders, ema

    def _voucher_mm(
        self, state: TradingState, mid_hist: Dict[str, List[float]]
    ) -> Dict[str, List[Order]]:
        orders: Dict[str, List[Order]] = {}
        for K, (pos_lim, quote_qty) in VEV_MM_CONFIG.items():
            sym = VEV[K]
            ...
            buy_cap = max(0, pos_lim - pos)
            sell_cap = max(0, pos_lim + pos)

            sym_orders: List[Order] = []
            if buy_cap > 0:
                sym_orders.append(Order(sym, fair - 1, min(quote_qty, buy_cap)))
            if sell_cap > 0:
                sym_orders.append(Order(sym, fair + 1, -min(quote_qty, sell_cap)))
