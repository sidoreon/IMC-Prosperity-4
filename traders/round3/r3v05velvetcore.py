# r3v05velvetcore.py: vNN counts only inside the `velvetcore` family (same round can have other r3v01… files with different tags).
...
class Trader(_R3V23HvoptVolBase):
    ...
    def _option_mispricing_orders(
        ...
        limit = min(self.POSITION_LIMITS[product], self.MAX_PER_STRIKE)
        buy_room = limit - pos
        sell_room = limit + pos

        edge_bid = best_bid - fair
        edge_ask = fair - best_ask

        if edge_ask > self.OPT_EDGE and buy_room > 0:
            qty = min(25, buy_room, abs(ask_vol))
            if qty > 0:
                out.append(Order(product, int(best_ask), int(qty)))
        if edge_bid > self.OPT_EDGE and sell_room > 0:
            qty = min(25, sell_room, abs(bid_vol))
            if qty > 0:
                out.append(Order(product, int(best_bid), -int(qty)))
        return out

    def _strike_monotonicity_violations(
        self,
        fairs: Dict[str, float],
    ) -> List[tuple[str, str]]:
        names = sorted(
            [p for p in fairs if p in self.VOUCHER_STRIKES],
            key=lambda p: self.VOUCHER_STRIKES[p],
        )
        bad: List[tuple[str, str]] = []
        for i in range(len(names) - 1):
            p1, p2 = names[i], names[i + 1]
            if fairs[p1] + self.ARB_MIN_GAP < fairs[p2]:
                bad.append((p1, p2))
        return bad

    def run(self, state):
        result, conv, trader_data = super().run(state)
        data = self._load_data(trader_data)

        extract_depth = state.order_depths.get("VELVETFRUIT_EXTRACT")
        extract_mid = self._mid(extract_depth) if extract_depth else None
        if extract_mid is None:
            return result, conv, trader_data
        ...
        for product in self.VOUCHER_STRIKES:
            ...
            if depth is None:
                continue
            fairs[product] = self._voucher_fair(product, extract_mid, state.timestamp, data)
            pos = state.position.get(product, 0)
            result.setdefault(product, [])
            result[product].extend(self._option_mispricing_orders(product, depth, pos, fairs[product]))

        violations = self._strike_monotonicity_violations(fairs)
        for low_k, high_k in violations:
            d_low = state.order_depths.get(low_k)
            d_high = state.order_depths.get(high_k)
            if d_low is None or d_high is None:
                continue
            low_bid, low_bid_v = self._best_bid(d_low)
            high_ask, high_ask_v = self._best_ask(d_high)
            if low_bid is None or high_ask is None:
                continue
            if low_bid > high_ask:
                qty = min(abs(low_bid_v), abs(high_ask_v), 15)
                if qty > 0:
                    result.setdefault(low_k, []).append(Order(low_k, int(low_bid), -int(qty)))
                    result.setdefault(high_k, []).append(Order(high_k, int(high_ask), int(qty)))
