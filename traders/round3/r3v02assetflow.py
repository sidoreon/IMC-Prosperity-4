# r3v02assetflow.py: vNN counts only inside the `assetflow` family (same round can have other r3v01… files with different tags).
...
class Trader(_R3V23HvoptVolBase):
    ...
    def _load_data(self, trader_data: str) -> dict:
        ...
        if "flow_ema_imbalance" not in data:
            data["flow_ema_imbalance"] = {}
        return data
    ...
    def _flow_orders(
        ...
        bid_v = float(abs(bid_vol))
        ask_v = float(abs(ask_vol))
        denom = max(1e-9, bid_v + ask_v)
        imbalance = (bid_v - ask_v) / denom

        old_ema = float(data["flow_ema_imbalance"].get(product, 0.0))
        ema = self.FLOW_ALPHA * imbalance + (1.0 - self.FLOW_ALPHA) * old_ema
        data["flow_ema_imbalance"][product] = ema

        mid = (bid + ask) / 2.0
        micro = (ask * bid_v + bid * ask_v) / denom
        signal = self.FLOW_W1 * ema + self.FLOW_W2 * (micro - mid)
        ...
        out: List[Order] = []
        if signal > self.FLOW_POS_TH and buy_room > 0:
            qty = min(self.FLOW_TAKE_SIZE, buy_room, int(ask_v))
            if position > 0:
                qty = max(1, qty // 2)
            if qty > 0:
                out.append(Order(product, int(ask), int(qty)))
        elif signal < self.FLOW_NEG_TH and sell_room > 0:
            qty = min(self.FLOW_TAKE_SIZE, sell_room, int(bid_v))
            if position < 0:
                qty = max(1, qty // 2)
            if qty > 0:
                out.append(Order(product, int(bid), -int(qty)))
        return out
    ...
    def _trade_delta_one(
        ...
        flow_orders = self._flow_orders(product, depth, position, data)
        return base_orders + flow_orders
