# r3v03velvetvol.py: vNN counts only inside the `velvetvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    # Version 1: Conservative voucher-implied underlying overlay. Adds a vega-weighted implied-spot extractor from vouche...
    ...
    def run(self, state: TradingState):
        ...
        if extract_fair is not None:
            ...
            if voucher_implied is not None:
                blended = 0.85 * extract_fair + 0.15 * voucher_implied
                max_dev = extract_fair * 0.015
                extract_fair = max(
    ...
    def _load_data(self, trader_data: str) -> dict:
        if not trader_data:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "last_voucher_implied": 0.0}
        try:
