# r3v24hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def _compute_ofi(self, depth: OrderDepth):
    ...
    def _trade_delta_one(self, product: str, depth: OrderDepth, position: int, limit: int, data: dict, fair: float) -> List[Order]:
        ...
        ofi = self._compute_ofi(depth)
        skew = max(-1, min(1, int(2 * ofi)))

        bid_px = max(best_bid, best_bid + skew)
        ask_px = min(best_ask, best_ask - skew)
