# r3v04hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product, od in ods.items():
            ...
            if product == INDEPENDENT_SPOT:
                orders, hydrogel_ema = self._hydrogel(od, pos, hydrogel_ema)
                res[product] = orders
                continue
    ...
    def _hydrogel(
        ...
        size = max(0, int(50 * (1.0 - abs(pos) / 200)))
        bid_size = min(size, max(0, 200 - pos))
        ask_size = min(size, max(0, 200 + pos))
        orders: List[Order] = []
        if bid_px < ask_px:
            if bid_size > 0:
                orders.append(Order(INDEPENDENT_SPOT, bid_px, bid_size))
            if ask_size > 0:
                orders.append(Order(INDEPENDENT_SPOT, ask_px, -ask_size))
        return orders, ema
