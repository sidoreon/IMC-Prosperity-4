# r4v10hvoptmark.py: vNN counts only inside the `hvoptmark` family (same round can have other r4v01… files with different tags).
...
class Trader:
    ...
    VF_CP_DECAY = 0.75
    VF_CP_CAP = 40.0
    VF_CP_FAIR_BIAS = 0.08
    VF_BULLISH_NAMES = {"Mark 67"}
    VF_BEARISH_NAMES = {"Mark 49"}
    ...
    def trade_velvetfruit_and_options(self, state: TradingState, result: dict, data: dict) -> None:
        ...
        MOMENTUM_SKEW = 0.1
        cp_signal = self._update_vf_counterparty_signal(state, data)
        signal = fair - mid - MOMENTUM_SKEW * momentum + self.VF_CP_FAIR_BIAS * cp_signal
        ...
        adjusted_fair = fair - INVENTORY_SKEW * position + MOMENTUM_SKEW * momentum + self.VF_CP_FAIR_BIAS * cp_signal
    ...
    def _update_vf_counterparty_signal(self, state: TradingState, data: dict) -> float:
        signal = float(data.get("vf_cp_signal", 0.0)) * self.VF_CP_DECAY
        market_trades = getattr(state, "market_trades", {}) or {}

        for trade in market_trades.get("VELVETFRUIT_EXTRACT", []):
            ...
            if buyer in self.VF_BULLISH_NAMES:
                signal += qty
            if seller in self.VF_BULLISH_NAMES:
                signal -= qty
            if buyer in self.VF_BEARISH_NAMES:
                signal -= qty
            if seller in self.VF_BEARISH_NAMES:
                signal += qty

        cap = self.VF_CP_CAP
        signal = max(-cap, min(cap, signal))
        data["vf_cp_signal"] = signal
        return signal