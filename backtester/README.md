...
Rust CLI for **local** IMC Prosperity 4 backtests (no hosted API in this tree).

## Layout in this monorepo

- **This directory** (`backtester/`): Rust crate, `Makefile`, `scripts/`, `LICENSE-APACHE`, `LICENSE-MIT`, and upstream docs below.
- **Repository root**: `datasets/`, `traders/`, `research/`, `runs/`, `tools/submit.py`, `requirements.txt`.

The binary resolves `datasets/` and `traders/` from the **repository root** automatically, so you can run `make tutorial` from `backtester/` without copying data into this folder.

Default trader when you omit `--trader`:

- `traders/latest_trader.py` (at repo root)
...
Or clone only the upstream backtester (historical paths):
...
- `backtester/src/` Rust implementation
- `backtester/scripts/` `cargo`/`doctor` wrappers (macOS)
- `traders/` Python strategies (includes `latest_trader.py` for defaults)
- `datasets/tutorial/` … `datasets/round8/` round CSVs and optional submission logs
- `runs/` backtest outputs (default output root is the repo root)
- `runs/<backtest-id>/` combined bundle when using `--persist` across days
