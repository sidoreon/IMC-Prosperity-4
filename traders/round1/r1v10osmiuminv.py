# r1v10osmiuminv.py: vNN counts only inside the `osmiuminv` family (same round can have other r1v01… files with different tags).
...
class OsmiumStrategy:
    ...
    def run(self, order_depth: OrderDepth, position: int, history: List[float], timestamp: int):
        ...
        ou_deviation = mid - rolling_mean

        ou_size_factor = abs(self.OU_BETA * ou_deviation) / max(vol, 1.0)
        take_size = max(4, min(int(ou_size_factor * 10), 20))

        predicted_next = 0.0
        if len(history) >= 1:
            predicted_next = self.LAG1_AUTOCORR * (mid - history[-1])

        lean_buy  = predicted_next >  self.AUTOCORR_THRESHOLD
        lean_sell = predicted_next < -self.AUTOCORR_THRESHOLD
        ...
        if spread >= self.MIN_PENNY_SPREAD:
            ...
            base_size = self.BASE_PASSIVE_SIZE
            if abs(z) > self.STRONG_TAKE_EDGE:
                base_size = self.BIG_PASSIVE_SIZE

            boosted = min(int(base_size * self.AUTOCORR_BOOST), self.BIG_PASSIVE_SIZE)

            if lean_buy:
                buy_size, sell_size = boosted, base_size
            elif lean_sell:
                buy_size, sell_size = base_size, boosted
            else:
            ...
            if buy_cap > 0:
                orders.append(Order(self.PRODUCT, bid_price, min(buy_size, buy_cap)))
            if sell_cap > 0:
                orders.append(Order(self.PRODUCT, ask_price, -min(sell_size, sell_cap)))
            ...
            bid_price = min(best_bid - skew, best_ask - 1)
            ask_price = max(best_ask - skew, best_bid + 1)
        ...
        lean_str = "lean_buy" if lean_buy else ("lean_sell" if lean_sell else "neutral")
        print(f"[OSMIUMGOAT] t={timestamp} mid={mid:.2f} fair={fair:.2f} z={z:.2f} dev={ou_deviation:.2f} pred={predicted_next:.3f} ts={take_size} pos={position} spread={spread} {lean_str}")
