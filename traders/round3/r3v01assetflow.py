# r3v01assetflow.py: vNN counts only inside the `assetflow` family (same round can have other r3v01… files with different tags).
# Cross-asset flow tilt.
#
...
# _R3V23HvoptVolBase — full r3v23hvoptvol trader logic (inlined; single-file, no cross-trader imports).
#
...
class Trader(_R3V23HvoptVolBase):
    FLOW_ALPHA = 0.20
    FLOW_W1 = 0.65
    FLOW_W2 = 0.35
    FLOW_TH = 0.10
    ...
    def _load_data(self, trader_data: str) -> dict:
        ...
        data.setdefault("meta_flow_ema", {})
        data.setdefault("meta_tox_jump_ema", {})
        data.setdefault("meta_spread_ema", {})
        return data

    def _flow_signal(self, product: str, depth: OrderDepth, data: dict) -> float:
        bid, bid_vol = self._best_bid(depth)
        ask, ask_vol = self._best_ask(depth)
        if bid is None or ask is None:
            return 0.0
        bv, av = float(abs(bid_vol)), float(abs(ask_vol))
        denom = max(1e-9, bv + av)
        imbalance = (bv - av) / denom
        old = float(data["meta_flow_ema"].get(product, 0.0))
        ema = self.FLOW_ALPHA * imbalance + (1.0 - self.FLOW_ALPHA) * old
        data["meta_flow_ema"][product] = ema
        micro = (ask * bv + bid * av) / denom
        mid = (bid + ask) / 2.0
        return self.FLOW_W1 * ema + self.FLOW_W2 * (micro - mid)

    def _tox_level(self, product: str, depth: OrderDepth, data: dict) -> int:
        mid = self._mid(depth)
        if mid is None:
            return 0
        prev = float(data["last_mid"].get(product, mid))
        jump = abs(mid - prev)
        old = float(data["meta_tox_jump_ema"].get(product, max(1e-6, jump)))
        avg = self.TOX_ALPHA * jump + (1.0 - self.TOX_ALPHA) * old
        data["meta_tox_jump_ema"][product] = max(1e-6, avg)
        ratio = jump / max(1e-6, avg)
        if ratio < 1:
            return 0
        if ratio < 2:
            return 1
    ...
    def _vol_multipliers(self, product: str, depth: OrderDepth, data: dict) -> Tuple[float, float]:
        bid, _ = self._best_bid(depth)
        ask, _ = self._best_ask(depth)
        mid = self._mid(depth)
        ...
        old_spread = float(data["meta_spread_ema"].get(product, spread))
        avg_spread = 0.1 * spread + 0.9 * old_spread
        data["meta_spread_ema"][product] = avg_spread
        regime = spread / max(1e-6, avg_spread)
        if regime < 0.9:
            return 0.9, 1.2
        if regime < 1.2:
            return 1.1, 1.0
    ...
    def _inventory_emergency_order(self, product: str, depth: OrderDepth, pos: int) -> List[Order]:
        limit = self.POSITION_LIMITS[product]
        pos_norm = pos / max(1.0, float(limit))
        if abs(pos_norm) <= 0.9:
            return []
        bid, bid_v = self._best_bid(depth)
        ask, ask_v = self._best_ask(depth)
        if bid is None or ask is None:
            return []
        if pos > 0:
            qty = min(pos, abs(bid_v), 30)
            return [Order(product, int(bid), -int(qty))] if qty > 0 else []
        qty = min(-pos, abs(ask_v), 30)
        return [Order(product, int(ask), int(qty))] if qty > 0 else []
    ...
    def _trade_delta_one(
        ...
        emergency = self._inventory_emergency_order(product, depth, position)
        if emergency:
            return emergency

        tox = self._tox_level(product, depth, data)
        if tox >= 3:
            return []

        flow = self._flow_signal(product, depth, data)
        spread_mult, size_mult = self._vol_multipliers(product, depth, data)
        ...
        fair_adj = fair
        try:
            self.VE_EDGE = max(1, int(round(old_edge * spread_mult)))
            self.VE_POST_EDGE = max(1, int(round(old_post * spread_mult)))
            self.VE_SIZE = max(1, int(round(old_size * size_mult)))
            self.VE_POST_SIZE = max(1, int(round(old_post_size * size_mult)))

            if tox == 2:
                self.VE_EDGE += 2
                self.VE_POST_EDGE += 2
                self.VE_SIZE = max(1, int(math.floor(self.VE_SIZE * 0.6)))
                self.VE_POST_SIZE = max(1, int(math.floor(self.VE_POST_SIZE * 0.6)))

            if flow > self.FLOW_TH:
                fair_adj += 1.5
            elif flow < -self.FLOW_TH:
                fair_adj -= 1.5

            return super()._trade_delta_one(product, depth, position, limit, data, fair_adj)
        finally:
