# r3v26hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if "VELVETFRUIT_EXTRACT" in state.order_depths and "VEV_5200" in state.order_depths:
            u = state.order_depths["VELVETFRUIT_EXTRACT"]
            if u.buy_orders and u.sell_orders:
                ubid, uask = max(u.buy_orders), min(u.sell_orders)
                mid = (ubid + uask) / 2

                prev = data.get("prev_mid", mid)
                move = abs(mid - prev)
                data["prev_mid"] = mid

                o = state.order_depths["VEV_5200"]
                if o.buy_orders and o.sell_orders:
                    obid, oask = max(o.buy_orders), min(o.sell_orders)
                    mid_o = (obid + oask) / 2

                    model = max(0, mid - 5200)
                    ...
                    if "VEV_5200" not in result: result["VEV_5200"] = []
                    if "VELVETFRUIT_EXTRACT" not in result: result["VELVETFRUIT_EXTRACT"] = []

                    if mid_o < model:
                        result["VEV_5200"].append(Order("VEV_5200", oask, size))
                        result["VELVETFRUIT_EXTRACT"].append(Order("VELVETFRUIT_EXTRACT", ubid, -int(0.6 * size)))
                    elif mid_o > model:
                        result["VEV_5200"].append(Order("VEV_5200", obid, -size))
                        result["VELVETFRUIT_EXTRACT"].append(Order("VELVETFRUIT_EXTRACT", uask, int(0.6 * size)))
