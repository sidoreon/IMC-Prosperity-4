# r3v04velvetvol.py: vNN counts only inside the `velvetvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    # Version 2: Moderate dual-model fusion with delta-bracket refinement. 1. Computes vega-weighted implied underlying f...
    ...
    VE_ANCHOR = 5252.288614621951
    VE_ANCHOR_WEIGHT = 0.537878371945186
    VE_EMA_ALPHA = 0.11489058752673036
    VE_IMBALANCE_K = 2.3265279646767905
    VE_IMBALANCE_CAP = 2.499806185525037
    ...
    def run(self, state: TradingState):
        ...
        voucher_underlying_fair = extract_fair
        if extract_fair is not None:

            voucher_implied = self._voucher_implied_underlying(
                data, state.order_depths, state.timestamp, extract_fair
            )
            if voucher_implied is not None:
                blended = 0.65 * extract_fair + 0.35 * voucher_implied
                max_dev = extract_fair * 0.025
                voucher_underlying_fair = max(
                    extract_fair - max_dev,
                    min(extract_fair + max_dev, blended)
                )
                data["last_voucher_implied"] = voucher_implied

            bracket_fair = self._delta_bracket_fair(
                data, state.order_depths, state.timestamp, voucher_underlying_fair
            )
            if bracket_fair is not None:
                refined = 0.70 * voucher_underlying_fair + 0.30 * bracket_fair
                max_dev2 = extract_fair * 0.030
                voucher_underlying_fair = max(
                    extract_fair - max_dev2,
                    min(extract_fair + max_dev2, refined)
                )
                data["last_bracket_fair"] = bracket_fair
        ...
        for product, depth in state.order_depths.items():
            ...
            elif product in self.VOUCHER_STRIKES and voucher_underlying_fair is not None:
                fair = self._voucher_fair(product, voucher_underlying_fair, state.timestamp, data)
                delta = self._voucher_delta(product, voucher_underlying_fair, state.timestamp, data)
                orders = self._trade_voucher(product, depth, pos, fair, voucher_underlying_fair, delta, data)
    ...
    def _vega(self, S: float, K: float, T: float, sigma: float, r: float = 0.0) -> float:
        if S <= 0 or K <= 0 or T <= 1e-8 or sigma <= 1e-8:
            return 0.0
        d1 = (math.log(S / K) + (r + 0.5 * sigma * sigma) * T) / (sigma * math.sqrt(T))
        return S * math.sqrt(T) * (1.0 / math.sqrt(2.0 * math.pi)) * math.exp(-0.5 * d1 * d1)

    def _implied_spot_from_voucher(self, market_price: float, K: float, T: float, sigma: float, r: float = 0.0) -> Optional[float]:
        if market_price <= 0 or K <= 0 or T <= 1e-8:
            return None
        lo, hi = 1.0, max(20000.0, K + market_price + 5000.0)
        price_at_hi = self._black_scholes_call(hi, K, T, sigma, r)
        if price_at_hi < market_price:
            return None
        price_at_lo = self._black_scholes_call(lo, K, T, sigma, r)
        if price_at_lo > market_price:
            return None
        for _ in range(30):
            mid = (lo + hi) / 2.0
            price = self._black_scholes_call(mid, K, T, sigma, r)
            if price < market_price:
                lo = mid
            else:
                hi = mid
    ...
    def _voucher_implied_underlying(self, data: dict, order_depths: Dict[str, OrderDepth], timestamp: int, current_underlying: float) -> Optional[float]:
        T = self._tte_years(timestamp)
        if T <= 1e-8 or current_underlying is None or current_underlying <= 0:
            return None
        ...
        for product, K in self.VOUCHER_STRIKES.items():
            ...
            sigma = self._sigma_for_product(data, product, current_underlying, timestamp)

            intrinsic = max(0.0, current_underlying - K)
            extrinsic = mid - intrinsic
            if extrinsic < 0.3 and current_underlying < K:
                continue

            implied_S = self._implied_spot_from_voucher(mid, K, T, sigma)
            if implied_S is None:
                continue

            vega = self._vega(implied_S, K, T, sigma)
            if vega < 0.01:
                continue

            delta = self._black_scholes_delta(implied_S, K, T, sigma)
            atm_weight = max(0.0, 1.0 - abs(delta - 0.5) * 2.0)
            if atm_weight <= 0:
                continue

            weight = vega * atm_weight
            implied_spots.append(implied_S)
        ...
        weighted_fair = sum(s * w for s, w in zip(implied_spots, weights)) / total_weight
        return weighted_fair

    def _delta_bracket_fair(self, data: dict, order_depths: Dict[str, OrderDepth], timestamp: int, base_underlying: float) -> Optional[float]:
        # Implements the user's bracketing logic using BS deltas. Finds adjacent strikes where delta crosses 0.5, uses (a+b)/...
        T = self._tte_years(timestamp)
        if T <= 1e-8 or base_underlying is None or base_underlying <= 0:
            return None
        ...
        for product, K in self.VOUCHER_STRIKES.items():
            ...
            sigma = self._sigma_for_product(data, product, base_underlying, timestamp)
            delta = self._black_scholes_delta(base_underlying, K, T, sigma)
            deltas.append(delta)
            strikes.append(K)
        ...
        paired = sorted(zip(strikes, deltas), key=lambda x: x[0])
        ...
        if lower_bracket is None or upper_bracket is None:
            return None

        base = (lower_bracket + upper_bracket) / 2.0

        total_weight = 0.0
        weighted_dev = 0.0

        for K, delta in paired:
            weight = max(0.01, abs(delta - 0.5))
            weighted_dev += (delta - 0.5) * weight
            total_weight += weight

        if total_weight <= 0:
            return base

        mean_dev = weighted_dev / total_weight

        adjustment = mean_dev * 120.0
    ...
    def _load_data(self, trader_data: str) -> dict:
        if not trader_data:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "last_voucher_implied": 0.0, "last_bracket_fair": 0.0}
        try:
            ...
            if "last_voucher_implied" not in data:
                data["last_voucher_implied"] = 0.0
            if "last_bracket_fair" not in data:
                data["last_bracket_fair"] = 0.0
            return data
        except Exception:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "last_voucher_implied": 0.0, "last_bracket_fair": 0.0}
