# r3v05velvetvol.py: vNN counts only inside the `velvetvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    # Version 3: Dynamic realized-derivative bracket (user's exact logic). Stores previous timestamp voucher and underlyi...
    ...
    def run(self, state: TradingState):
        ...
        self._store_voucher_history(data, state.order_depths, state.timestamp)
        ...
        derivative_fair = None
        if extract_fair is not None:
            derivative_fair = self._derivative_bracket_fair(
                data, state.order_depths, state.timestamp, extract_fair
            )
            if derivative_fair is not None:
                max_dev = extract_fair * 0.012
                derivative_fair = max(
                    extract_fair - max_dev,
                    min(extract_fair + max_dev, derivative_fair)
                )
                data["last_derivative_fair"] = derivative_fair
                data["using_derivative_fair"] = True
            else:
                data["using_derivative_fair"] = False
        else:
            data["using_derivative_fair"] = False
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "VELVETFRUIT_EXTRACT":
                ...
                underlying_for_voucher = derivative_fair if derivative_fair is not None else extract_fair
                fair = self._voucher_fair(product, underlying_for_voucher, state.timestamp, data)
                delta = self._voucher_delta(product, underlying_for_voucher, state.timestamp, data)
                orders = self._trade_voucher(product, depth, pos, fair, underlying_for_voucher, delta, data)
    ...
    def _store_voucher_history(self, data: dict, order_depths: Dict[str, OrderDepth], timestamp: int) -> None:
        # Store current voucher and underlying mids for next timestamp's derivative calc.
        history = {}
        ...
        underlying_depth = order_depths.get("VELVETFRUIT_EXTRACT")
        if underlying_depth is not None:
            underlying_mid = self._mid(underlying_depth)
            if underlying_mid is not None:
                history["VELVETFRUIT_EXTRACT"] = underlying_mid

        if history:
            data["prev_voucher_snapshot"] = history
            data["prev_timestamp"] = timestamp

    def _derivative_bracket_fair(self, data: dict, order_depths: Dict[str, OrderDepth], timestamp: int, current_underlying: float) -> Optional[float]:
        # User's exact logic: 1. Compute realized delta = Δvoucher / Δunderlying from stored history.
        prev_snapshot = data.get("prev_voucher_snapshot")
        prev_timestamp = data.get("prev_timestamp")

        if prev_snapshot is None or prev_timestamp is None:
            return None

        if timestamp <= prev_timestamp:
            return None

        prev_underlying = prev_snapshot.get("VELVETFRUIT_EXTRACT")
        if prev_underlying is None or current_underlying is None:
            return None

        underlying_change = current_underlying - prev_underlying
        if abs(underlying_change) < 0.5:
            return None
        ...
        for product, K in self.VOUCHER_STRIKES.items():
            prev_mid = prev_snapshot.get(product)
            curr_depth = order_depths.get(product)
            if prev_mid is None or curr_depth is None:
                continue
            curr_mid = self._mid(curr_depth)
            if curr_mid is None:
                continue

            deriv = (curr_mid - prev_mid) / underlying_change

            if deriv < -0.1 or deriv > 1.1:
                continue
        ...
        paired = sorted(zip(strikes, derivatives), key=lambda x: x[0])
        ...
        for K, deriv in paired:
            weight = max(0.01, abs(deriv - 0.5))
            weighted_dev += (deriv - 0.5) * weight
            total_weight += weight
    ...
    def _load_data(self, trader_data: str) -> dict:
        if not trader_data:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "last_derivative_fair": 0.0, "using_derivative_fair": False}
        try:
            ...
            if "last_derivative_fair" not in data:
                data["last_derivative_fair"] = 0.0
            if "using_derivative_fair" not in data:
                data["using_derivative_fair"] = False
            return data
        except Exception:
            return {"ema_fast": {}, "ema_slow": {}, "last_mid": {}, "ret_ema": {}, "bs_sigma": 0.24, "bs_sigma_by_product": {}, "smile_residual": 0.0, "smile_residual_by_product": {}, "cash": {}, "seen_trade_keys": [], "peak_pnl": 0.0, "last_total_pnl": 0.0, "last_derivative_fair": 0.0, "using_derivative_fair": False}
