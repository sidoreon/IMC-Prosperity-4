# r3v29hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader(BaseTrader):
    ...
    IMPACT_BASE_DEPTH = 30.0
    VF_OFI_K = 1.2
    VF_OFI_CAP = 2.0

    def _load_data(self, trader_data: str) -> dict:
        data = super()._load_data(trader_data)
        data.setdefault("vol_ema_abs_ret", {})
        data.setdefault("spread_ema", {})
        data.setdefault("tox_jump_ema", {})
        return data

    def _market_state(self, product: str, depth: OrderDepth, data: dict) -> Tuple[float, float, float, bool]:
        mid = self._mid(depth)
        bid, bid_vol = self._best_bid(depth)
        ask, ask_vol = self._best_ask(depth)
        ...
        spread_ratio = spread / max(1e-6, avg_spread)
        ...
        extreme_spike = abs_ret > self.SPIKE_TH * max(1e-6, vol)
        _ = bid_vol, ask_vol
        return vol, spread_ratio, toxicity, extreme_spike

    def _vol_multipliers(self, vol: float, spread_ratio: float) -> Tuple[float, float]:

        adjusted_spread = self.BASE_SPREAD + self.K1 * vol + self.K2 * spread_ratio
        if adjusted_spread < 3.0:
            return 0.95, 1.10
        if adjusted_spread < 5.0:
            return 1.05, 1.0
        return 1.2, 0.8

    def _vf_ofi_lean(self, depth: OrderDepth) -> float:
        bid, bid_vol = self._best_bid(depth)
        ask, ask_vol = self._best_ask(depth)
        if bid is None or ask is None:
            return 0.0
        ...
        tot = max(1e-9, a + b)
        ofi = (b - a) / tot
        return self._clip(self.VF_OFI_K * ofi, -self.VF_OFI_CAP, self.VF_OFI_CAP)

    def _impact_size_scale(self, depth: OrderDepth) -> float:
        _, bid_vol = self._best_bid(depth)
        _, ask_vol = self._best_ask(depth)
        touch_depth = max(1.0, float(abs(bid_vol) + abs(ask_vol)))

        return self._clip(math.sqrt(touch_depth / self.IMPACT_BASE_DEPTH), 0.65, 1.40)

    def _inventory_overlay(
        self,
        product: str,
        depth: OrderDepth,
        position: int,
        ...
        pos_norm = position / float(limit)
        skew = self.INV_SKEW_MAX * math.tanh(pos_norm / self.INV_SCALE)
        urgency = abs(pos_norm)
        ...
        if not force_only:
            for o in raw_orders:
                shifted.append(Order(o.symbol, int(round(o.price - skew)), o.quantity))

        if urgency > self.INV_EMERGENCY_TH:
            if position > 0:
                qty = min(position, self.INV_FORCE_SIZE, abs(bid_vol))
                return [Order(product, int(best_bid), -int(qty))] if qty > 0 else []
            if position < 0:
                qty = min(-position, self.INV_FORCE_SIZE, abs(ask_vol))
                return [Order(product, int(best_ask), int(qty))] if qty > 0 else []
            return []

        if urgency > self.INV_FORCE_TH:
            if position > 0:
                qty = min(position, self.INV_FORCE_SIZE, abs(bid_vol))
                if qty > 0:
                    shifted.append(Order(product, int(best_bid), -int(qty)))
            elif position < 0:
                qty = min(-position, self.INV_FORCE_SIZE, abs(ask_vol))
                if qty > 0:
                    shifted.append(Order(product, int(best_ask), int(qty)))

        return shifted

    def _trade_hydrogel(self, depth: OrderDepth, position: int, data: dict) -> List[Order]:
        base_orders = super()._trade_hydrogel(depth, position, data)
        vol, spread_ratio, toxicity, extreme_spike = self._market_state("HYDROGEL_PACK", depth, data)
        spread_mult, size_mult = self._vol_multipliers(vol, spread_ratio)
        impact_scale = self._impact_size_scale(depth)
        reduce_scale = size_mult * impact_scale

        adjusted: List[Order] = []
        for o in base_orders:
            q = int(math.copysign(max(1, int(round(abs(o.quantity) * reduce_scale))), o.quantity))
            px = o.price
            if abs(o.quantity) >= self.HG_TAKE_SIZE:

                if toxicity >= self.TOX_HIGH:
                    continue

            if o.quantity > 0:
                px = int(round(px - max(0.0, spread_mult - 1.0)))
            else:
                px = int(round(px + max(0.0, spread_mult - 1.0)))
            adjusted.append(Order(o.symbol, px, q))

        force_only = extreme_spike or toxicity >= self.TOX_EXTREME
        return self._inventory_overlay("HYDROGEL_PACK", depth, position, adjusted, force_only=force_only)
    ...
    def _trade_delta_one(
        ...
        vol, spread_ratio, toxicity, extreme_spike = self._market_state(product, depth, data)
        spread_mult, size_mult = self._vol_multipliers(vol, spread_ratio)
        impact_scale = self._impact_size_scale(depth)

        if extreme_spike or toxicity >= self.TOX_EXTREME:
            return self._inventory_overlay(product, depth, position, [], force_only=True)

        fair_adj = fair
        if product == "VELVETFRUIT_EXTRACT":

            fair_adj += self._vf_ofi_lean(depth)

        old_edge = self.VE_EDGE
        old_post = self.VE_POST_EDGE
        old_size = self.VE_SIZE
        old_post_size = self.VE_POST_SIZE
        try:
            tox_widen = 1.0 if toxicity < self.TOX_HIGH else 1.35
            self.VE_EDGE = max(1, int(round(old_edge * spread_mult * tox_widen)))
            self.VE_POST_EDGE = max(1, int(round(old_post * spread_mult * tox_widen)))
            size_factor = size_mult * impact_scale * (0.7 if toxicity >= self.TOX_HIGH else 1.0)
            self.VE_SIZE = max(1, int(round(old_size * size_factor)))
            self.VE_POST_SIZE = max(1, int(round(old_post_size * size_factor)))
            base_orders = super()._trade_delta_one(product, depth, position, limit, data, fair_adj)
        finally:
            self.VE_EDGE = old_edge
            self.VE_POST_EDGE = old_post
            self.VE_SIZE = old_size
            self.VE_POST_SIZE = old_post_size

        return self._inventory_overlay(product, depth, position, base_orders, force_only=False)
    ...
    def _trade_voucher(
        ...
        base_orders = super()._trade_voucher(product, depth, pos, fair, underlying_fair, delta, data)

        limit = self.POSITION_LIMITS[product]
        pos_norm = abs(pos) / max(1.0, float(limit))
        strike = self.VOUCHER_STRIKES.get(product, 0)
        ...
        if strike >= 5500:
            trimmed: List[Order] = []
            for o in base_orders:
                q = int(math.copysign(max(1, abs(o.quantity) // 3), o.quantity))
                trimmed.append(Order(o.symbol, o.price, q))
            base_orders = trimmed
        elif strike in (5200, 5300):
            boosted: List[Order] = []
            for o in base_orders:
                q = int(math.copysign(max(1, int(round(abs(o.quantity) * 1.35))), o.quantity))
                boosted.append(Order(o.symbol, o.price, q))
            base_orders = boosted
        ...
        unwind: List[Order] = []
        if pos > 0:
            qty = min(int(abs(pos) * 0.22), abs(bid_vol), 18)
            if qty > 0:
                unwind.append(Order(product, int(best_bid), -int(qty)))
        elif pos < 0:
            qty = min(int(abs(pos) * 0.22), abs(ask_vol), 18)
            if qty > 0:
                unwind.append(Order(product, int(best_ask), int(qty)))

        return base_orders + unwind
