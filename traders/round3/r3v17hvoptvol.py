# r3v17hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "VELVETFRUIT_EXTRACT":
                ...
                orders = self._trade_wing_mm(product, depth, pos)
    ...
    def _trade_wing_mm(self, product: str, depth: OrderDepth, pos: int) -> List[Order]:
        # Market-make VEV_6000 / VEV_6500 at market prices. Quote bid size=20 and ask size=15 at the existing best bid/ask.
        orders: List[Order] = []
        limit = self.POSITION_LIMITS[product]
        buy_cap = limit - pos
        sell_cap = limit + pos
        ...
        if buy_cap > 0:
            orders.append(Order(product, int(best_bid), min(20, buy_cap)))

        if sell_cap > 0:
            orders.append(Order(product, int(best_ask), -min(15, sell_cap)))
