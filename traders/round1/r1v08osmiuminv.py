# r1v08osmiuminv.py: vNN counts only inside the `osmiuminv` family (same round can have other r1v01… files with different tags).
...
class OsmiumStrategy:
    ...
    def run(self, order_depth: OrderDepth, position: int, history: List[float], timestamp: int):
        ...
        print(f"[OSMIUMv5] t={timestamp} mid={mid:.2f} fair={fair:.2f} z={z:.2f} acl={autocorr_lean} pos={position} spread={spread}")
