# r3v01assetema.py: vNN counts only inside the `assetema` family (same round can have other r3v01… files with different tags).
...
class Trader(_R3V23HvoptVolBase):
    ...
    def _load_data(self, trader_data: str) -> dict:
        ...
        if "tox_jump_ema" not in data:
            data["tox_jump_ema"] = {}
        return data

    def _toxicity_level(self, product: str, depth: OrderDepth, data: dict) -> int:
        mid = self._mid(depth)
        if mid is None:
            return 0
        prev_mid = float(data["last_mid"].get(product, mid))
        jump = abs(mid - prev_mid)
        old = float(data["tox_jump_ema"].get(product, max(1e-6, jump)))
        avg_jump = self.TOX_ALPHA * jump + (1.0 - self.TOX_ALPHA) * old
        data["tox_jump_ema"][product] = max(1e-6, avg_jump)
        toxicity = jump / max(1e-6, avg_jump)
    ...
    def _trade_delta_one(
        ...
        level = self._toxicity_level(product, depth, data)
        ...
        try:
            if level == 1:
                self.VE_POST_EDGE = old_post + 1
            elif level == 2:
                self.VE_EDGE = old_edge + 2
                self.VE_POST_EDGE = old_post + 2
                self.VE_SIZE = max(1, int(round(old_size * 0.6)))
                self.VE_POST_SIZE = max(1, int(round(old_post_size * 0.6)))
