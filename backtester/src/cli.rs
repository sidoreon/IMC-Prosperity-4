...
use crate::runner::{
    default_output_root, display_path, project_root, run_backtest, workspace_root,
};
...
fn resolve_dataset_input(requested: Option<&str>) -> Result<ResolvedDataset> {
    resolve_dataset_input_with_root(requested, &workspace_root().join("datasets"))
}
...
fn candidate_trader_roots() -> Result<Vec<PathBuf>> {
    ...
    for base in [workspace, crate_dir] {
...
mod tests {
    ...
    fn tutorial_dataset_returns_single_day() {
        let dataset =
            load_dataset(&workspace_root().join("datasets/tutorial/prices_round_0_day_-1.csv"))
    ...
    fn explicit_dataset_path_does_not_require_populated_default_datasets_root() {
        ...
        let explicit_dataset = workspace_root().join("datasets/tutorial");
    ...
    fn carry_plan_groups_tutorial_days_but_leaves_submission_separate() {
        let (_run_id, plans) = build_run_plan(
            &[workspace_root().join("datasets/tutorial")],
            None,
            Some("carry-check"),
            true,
            true,
    ...
    fn carry_dataset_normalizes_day_boundaries_into_continuous_timestamps() {
        let dataset = build_carry_dataset(&[
            (
                workspace_root().join("datasets/tutorial/prices_round_0_day_-2.csv"),
                Some(-2),
            ),
            (
                workspace_root().join("datasets/tutorial/prices_round_0_day_-1.csv"),
                Some(-1),
            ),
        ])
