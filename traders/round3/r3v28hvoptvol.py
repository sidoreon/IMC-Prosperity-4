# r3v28hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader(BaseTrader):
    ...
    def _load_data(self, trader_data: str) -> dict:
        ...
        if "vol_ema_abs_ret" not in data:
            data["vol_ema_abs_ret"] = {}
        if "spread_ema" not in data:
            data["spread_ema"] = {}
        return data

    def _vol_spread_size_multipliers(
        self,
        product: str,
        depth: OrderDepth,
        data: dict,
        ...
        extreme = abs_ret > self.SPIKE_TH * max(1e-6, vol)

        if adjusted_spread < 3.0:
            return 0.9, 1.3, extreme
        if adjusted_spread < 5.0:
            return 1.2, 1.0, extreme
        return 1.8, 0.45, extreme
    ...
    def _inventory_overlay(
        ...
        shifted: List[Order] = []
        for o in raw_orders:
            px = int(round(o.price - skew))
            shifted.append(Order(o.symbol, px, o.quantity))
    ...
    def _trade_delta_one(
        ...
        spread_mult, size_mult, extreme = self._vol_spread_size_multipliers(product, depth, data)
        if extreme:
            return self._inventory_overlay(product, depth, position, [])
        ...
        try:
            ...
            base_orders = super()._trade_delta_one(product, depth, position, limit, data, fair)
        finally:
        ...
        return self._inventory_overlay(product, depth, position, base_orders)
