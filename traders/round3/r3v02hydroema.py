# r3v02hydroema.py: vNN counts only inside the `hydroema` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, od in state.order_depths.items():
            ...
            lim = self.POSITION_LIMITS[product]
            orders, mem = self._trade_hydrogel(od, pos, lim, mem)
    ...
    def _trade_hydrogel(
        ...
        if spread < self.MIN_MAKE_SPREAD:
            return orders, mem
        ...
        buy_cap = lim - pos
        sell_cap = lim + pos
        size = max(5, int(self.HYDROGEL_SIZE * (1.0 - abs(pos) / lim)))
        if buy_cap > 0:
