# r3v33hvoptvol.py: vNN counts only inside the `hvoptvol` family (same round can have other r3v01… files with different tags).
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            if product == "HYDROGEL_PACK":
                orders = self._trade_hydrogel(depth, pos, data, state.timestamp)
    ...
    def _trade_hydrogel(self, depth: OrderDepth, position: int, data: dict, timestamp: int = 0) -> List[Order]:
        orders: List[Order] = []
        ...
        bsz = abs(depth.buy_orders[bb])
        asz = abs(depth.sell_orders[ba])
        tot = bsz + asz
        ...
        expected = ema + 0.30 * (micro - ema) + mr_off - inv_skew

        base_make = 4.0 * 0.75
        make_edge = base_make + 0.08 * max(vol, 0.0)
        if (int(timestamp) % 1_000_000) > 990_000:
            make_edge *= 1.25
        ...
        ms = 5
        ms_buy  = ms + 1 if position < 0 else ms
        ms_sell = ms + 1 if position > 0 else ms
